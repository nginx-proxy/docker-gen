...
dist: dist-clean
	...
	mkdir -p dist/alpine-linux/ppc64le && GOOS=linux GOARCH=ppc64le go build -ldflags "$(LDFLAGS)" -a -tags netgo -installsuffix netgo -o dist/alpine-linux/ppc64le/docker-gen ./cmd/docker-gen
	...
	mkdir -p dist/linux/ppc64le  && GOOS=linux GOARCH=ppc64le go build -ldflags "$(LDFLAGS)" -o dist/linux/ppc64le/docker-gen ./cmd/docker-gen
...
release: dist
	...
	tar -cvzf docker-gen-alpine-linux-ppc64le-$(TAG).tar.gz -C dist/alpine-linux/ppc64le docker-gen
	...
	tar -cvzf docker-gen-linux-ppc64le-$(TAG).tar.gz -C dist/linux/ppc64le docker-gen
